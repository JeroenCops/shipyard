    ...
    match fields {
        syn::Fields::Named(fields) => {
            ...
            let field = field_name
                ...
                .map(|((field_name, field_type), field_is_default)| {
                    if field_is_default {
                        ...
                        quote!(
                            #field_name: <#field_type as ::shipyard::WorldBorrow>::world_borrow(world, system_id, last_run, current)?
                        )
                    }
                });
            ...
            Ok(quote!(
                impl #impl_generics ::shipyard::WorldBorrow for #name #ty_generics #where_clause {
                    ...
                    fn world_borrow<'__w>(world: & '__w ::shipyard::World, system_id: Option<::shipyard::info::TypeId>, last_run: Option<u32>, current: u32) -> Result<Self::WorldView<'__w>, ::shipyard::error::GetStorage> {
                        Ok(#name {
                            #(#field),*
                        })
                    }
        ...
        syn::Fields::Unnamed(fields) => {
            let world_borrow = fields.unnamed.iter().map(|field| {
                ...
                quote!(<#field_type as ::shipyard::WorldBorrow>::world_borrow(world, system_id, last_run, current)?)
            });
